...
const generateMeetingLink = async () => {
  ...
  const endDateTime = new Date(localDateTime.getTime() + durationInMinutes * 60000);

  try {
    const response = await axios.post('http://localhost:5050/api/create-meeting', {
      summary: `${formData.subject} Session with ${formData.tutor}`,
      startTime: localDateTime.toISOString(),
      endTime: endDateTime.toISOString(),
      timeZone: userTimeZone,
      // Send additional info for debugging
      localDateTimeString: dateStr,
  ...
  type='button'
  variant='outline'
  onClick={async () => await generateMeetingLink()}
