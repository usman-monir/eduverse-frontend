  ...
  const getFilteredSessions = () => {
    ...
    if (user?.role === 'tutor') {
      ...
      filtered = sessions.filter(session => {
        ...
        const isCreator = session.createdBy?.toString() === user._id?.toString() || session.createdBy === user._id;
      ...
      <div className='space-y-6'>
        ...
        <div className='flex items-center justify-between'>
          <div>
            ...
            <p className='text-gray-600'>
              {user?.role === 'admin' ? 'Manage all class sessions' : 'Manage your sessions and requests'}
            </p>
          </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className='text-2xl font-bold'>
                {filteredSessions.filter(s => s.status === 'available').length}
              </div>
        ...
        <Card>
          <CardHeader>
                ...
                <CardDescription>
                  {user?.role === 'admin' ? 'All sessions in the system' : 'Your assigned and created sessions'}
                </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <CardContent>
                <div className='space-y-4'>
                  {filteredSessions.filter(s => 
                    ...
                    filteredSessions
                      .filter(s => s.createdBy?.toString() === user?._id?.toString() || s.createdBy === user?._id)
                      .map((session) => (
                        <div
                          key={session.id}
                          className='flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50'
          ...
          <TabsContent value='slot-requests' className='space-y-4'>
            <Card>
              ...
              <CardContent>
                <div className='space-y-4'>
                  {filteredSessions.filter(s => s.type === 'slot_request').length > 0 ? (
                    filteredSessions
